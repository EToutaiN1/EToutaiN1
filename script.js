...
class App {
  constructor() {
    ...
    let fov =
      2 *
      Math.atan(this.planeHeight / (2 * this.cameraOpts.z)) *
      (180 / Math.PI);
  ...
  start() {
    ...
    let texture = new THREE.Texture(
      this.createCanvas(
        "Oddly Enough is a production house for the post digital era. With our gaze set on the future of content, we aim to produce without distinction for the physical, digital and virtual realms. Before anything else, we are a community of creative forces who strive for uniqueness. We believe that in an era of never ending content, the best thing to be is the odd one out."
      )
    );
  ...
  debug() {
    let gridHelper = new THREE.GridHelper(
      APP_CONFIG.gridDebugSize.x,
      APP_CONFIG.gridDebugSize.y
    );
